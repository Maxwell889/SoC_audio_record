...
#define RECORD_FLASH_MAX_PAGES     (1024UL)

#define RUN_UART_SAMPLE_WAV_FLASH_TEST (0U)
#define RUN_ADPCM_ONLY_DEBUG           (0U)
...
#define REC_ERR_AUDIO_TIMEOUT      (0x00000001UL)
#define REC_ERR_FLASH_TIMEOUT      (0x00000002UL)
#define REC_ERR_PAGE_VERIFY        (0x00000004UL)
#define AUDIO_WORD_WAIT_LIMIT      (5000000UL)
#define FLASH_READY_WAIT_LIMIT     (100000UL)
#define SPI_XFER_WAIT_LIMIT        (2000000UL)

/*
 * Keep debug result words in normal .bss storage. The previous fixed
 * address 0x00008000 is the Keil RW_IRAM1 base, so writing done_* there
 * overwrote main.o globals such as g_fifo_max_usedw/g_record_wav_crc.
 */
static volatile uint32_t done_flag_storage;
static volatile uint32_t done_size_storage;
static volatile uint32_t done_jedec_id_storage;
static volatile uint32_t done_uart_state_storage;
static volatile uint32_t done_source_crc_storage;
static volatile uint32_t done_flash_crc_storage;
static volatile uint32_t done_fifo_max_storage;

static volatile uint32_t * const done_flag = &done_flag_storage;
static volatile uint32_t * const done_size = &done_size_storage;
static volatile uint32_t * const done_jedec_id = &done_jedec_id_storage;
static volatile uint32_t * const done_uart_state = &done_uart_state_storage;
static volatile uint32_t * const done_source_crc = &done_source_crc_storage;
static volatile uint32_t * const done_flash_crc = &done_flash_crc_storage;
static volatile uint32_t * const done_fifo_max = &done_fifo_max_storage;

static uint8_t flash_page_buf[SPI_FLASH_PAGE_BYTES];
static uint8_t flash_verify_buf[SPI_FLASH_PAGE_BYTES];
static uint8_t flash_verify_buf2[SPI_FLASH_PAGE_BYTES];
static uint8_t adpcm_debug_block[ADPCM_BLOCK_ALIGN];
static uint32_t record_source_page_crc[RECORD_FLASH_MAX_PAGES];
static uint32_t record_source_page_len[RECORD_FLASH_MAX_PAGES];
static uint32_t flash_page_addr;
static uint32_t flash_page_len;
static uint32_t g_record_source_page_count;
static uint32_t g_record_page_crc_fail_count;
static uint32_t g_record_first_bad_page;
static uint32_t g_record_first_bad_offset;
static uint32_t g_record_first_bad_len;
static uint32_t g_record_first_bad_source_crc;
static uint32_t g_record_first_bad_flash_crc;
static uint32_t g_fifo_max_usedw;
static uint32_t g_record_wav_crc;
static uint32_t g_record_adpcm_crc;
static uint32_t g_record_error_flags;
static uint32_t g_record_error_block;
static uint32_t g_record_error_word;
static uint32_t g_page_verify_fail_count;
static uint32_t g_page_verify_first_offset;
static uint32_t g_page_verify_first_expected;
static uint32_t g_page_verify_first_actual;
static uint32_t g_page_verify_first_missing_zero;
static uint32_t g_page_verify_first_unwanted_zero;
static uint32_t g_page_verify_read_unstable_count;
static uint32_t g_page_verify_first_actual2;
static uint32_t g_debug_erase_bad_count;
static uint32_t g_debug_first_mismatch_offset;
static uint32_t g_debug_first_expected;
static uint32_t g_debug_first_actual;
static uint32_t g_debug_first_actual2;
static uint32_t g_debug_first_wren_sr;
static uint32_t g_debug_first_pp_sr;
static uint32_t g_debug_first_wait_sr;
static uint32_t g_debug_jedec0;
static uint32_t g_debug_jedec1;
static uint32_t g_debug_jedec2;
static uint32_t g_debug_jedec3;
static uint32_t g_debug_jedec4;
static uint32_t g_debug_jedec5;
static uint32_t g_debug_jedec6;
static uint32_t g_debug_jedec7;
static uint32_t g_debug_sr_before_clear;
static uint32_t g_debug_sr_after_clear;
...
static uint8_t audio_read_adpcm_word_timeout(uint32_t *word)
{
    uint32_t wait;

    wait = AUDIO_WORD_WAIT_LIMIT;
    while ((AUDIO_STATUS & AUDIO_STATUS_FIFO_EMPTY) != 0UL) {
        if (wait == 0UL) {
            *word = 0UL;
            return 0U;
...
static void spi0_write8(uint8_t tx)
{
    ...
    wait = SPI_XFER_WAIT_LIMIT;
    while ((SPI_SR & SPI_SR_TNF) == 0UL) {
        if (wait == 0UL) {
            g_record_error_flags |= REC_ERR_FLASH_TIMEOUT;
            return;
        }
        wait--;
    }
...
static uint8_t w25_read_status1(void)
{
    ...
    sr = rx[1];
    for (i = 1UL; i < sizeof(rx); ++i) {
        if ((rx[i] != 0xFFU) && (rx[i] != 0xFEU)) {
            sr = rx[i];
            break;
        }
    }
...
static void w25_read_jedec_raw_debug(void)
{
    uint8_t tx[8];
    uint8_t rx[8];
    uint32_t i;
...
static void w25_wait_ready(void)
{
    ...
    wait = FLASH_READY_WAIT_LIMIT;
    while (1) {
        sr1 = w25_read_status1();
        if ((sr1 & W25_SR_BUSY) == 0U) {
            /*
             * Double-check: the MISO pipeline can return stale bytes with
             * BUSY=0 right after a page-program or erase command, while the
             * flash is still internally busy.  A second read a few cycles
             * later gives the pipeline time to flush and returns the real
             * status.  Only accept "ready" when both reads agree.
             */
            delay_cycles(200UL);
            sr2 = w25_read_status1();
            if ((sr2 & W25_SR_BUSY) == 0U) {
                return;
            }
        }
        if (wait == 0UL) {
...
static void w25_clear_protection(void)
{
    ...
    g_debug_sr_before_clear = w25_read_status1();
    if ((g_debug_sr_before_clear & W25_SR_BP_MASK) == 0U) {
        g_debug_sr_after_clear = g_debug_sr_before_clear;
        return;
    }
    ...
    w25_write_disable();
    delay_cycles(5000UL);
    g_debug_sr_after_clear = w25_read_status1();
}
...
static void spi_flash_page_program(uint32_t addr, const uint8_t *data, uint32_t len)
{
    ...
    while (offset < len) {
        ...
        if (offset == 0UL) {
            g_debug_first_pp_sr = w25_read_status1();
        }
        w25_wait_ready();
        if (offset == 0UL) {
            g_debug_first_wait_sr = w25_read_status1();
        }
        offset += chunk;
    }
}
...
static void spi_flash_dummy_read(uint32_t addr)
{
    uint32_t len;

    len = min_u32(8UL, SPI_FLASH_READ_CHUNK_BYTES);
    spi_flash_read(addr, flash_verify_buf, len);
}

static void spi_flash_read_stable(uint32_t addr, uint8_t *data, uint32_t len)
{
    uint32_t i;
    uint32_t retry;

    /*
     * The MISO pipeline can return stale bits, making two consecutive
     * reads of the same address disagree.  Keep reading (dummy + two
     * full reads) until the two full reads agree byte-for-byte.  This
     * is critical for ACRC / PCRC correctness — a single unstable read
     * poisons the CRC.
     */
    for (retry = 0UL; retry < 5UL; ++retry) {
        spi_flash_dummy_read(addr);
        spi_flash_read(addr, flash_verify_buf, len);
        spi_flash_read(addr, data, len);

        for (i = 0UL; i < len; ++i) {
            if (flash_verify_buf[i] != data[i]) {
                break;
            }
        }
        if (i == len) {
            return;   /* both reads agree — stable */
        }
    }
    /* Exhausted retries; use the last read.  Caller should handle. */
}
...
static void flash_stream_begin(uint32_t addr)
{
    ...
    flash_page_len = 0UL;
    g_record_wav_crc = 0xFFFFFFFFUL;
    g_record_source_page_count = 0UL;
    g_record_page_crc_fail_count = 0UL;
    g_record_first_bad_page = 0xFFFFFFFFUL;
    g_record_first_bad_offset = 0xFFFFFFFFUL;
    g_record_first_bad_len = 0UL;
    g_record_first_bad_source_crc = 0UL;
    g_record_first_bad_flash_crc = 0UL;
    g_page_verify_fail_count = 0UL;
    g_page_verify_first_offset = 0xFFFFFFFFUL;
    g_page_verify_first_expected = 0UL;
    g_page_verify_first_actual = 0UL;
    g_page_verify_first_missing_zero = 0UL;
    g_page_verify_first_unwanted_zero = 0UL;
    g_page_verify_read_unstable_count = 0UL;
    g_page_verify_first_actual2 = 0UL;
}

static void flash_stream_verify_page(uint32_t addr,
                                     const uint8_t *expected,
                                     uint32_t len)
{
    uint32_t i;
    uint8_t first_differs;

    /*
     * main_flash.c found that the first burst read after back-to-back page
     * programs can occasionally return stale bits on this PL022/Flash path.
     * Do a tiny aligned read first, then two full reads for comparison.
     *
     * Read-instability policy: if the two reads disagree but ONE of them
     * matches the expected byte, the data was correctly programmed (reads
     * are non-destructive — a read cannot show correct data that was never
     * written).  Only fail when NEITHER read matches expected.
     */
    spi_flash_dummy_read(addr);
    spi_flash_read(addr, flash_verify_buf, len);
    spi_flash_read(addr, flash_verify_buf2, len);

    for (i = 0UL; i < len; ++i) {
        first_differs = (flash_verify_buf[i] != flash_verify_buf2[i]) ? 1U : 0U;
        if (first_differs) {
            if (g_page_verify_read_unstable_count == 0UL) {
                g_page_verify_first_offset = (addr - SPI_FLASH_WAV_ADDR) + i;
                g_page_verify_first_expected = expected[i];
                g_page_verify_first_actual = flash_verify_buf[i];
                g_page_verify_first_actual2 = flash_verify_buf2[i];
                g_page_verify_first_missing_zero = flash_verify_buf[i] & (~expected[i] & 0xFFU);
                g_page_verify_first_unwanted_zero = expected[i] & (~flash_verify_buf[i] & 0xFFU);
            }
            g_page_verify_read_unstable_count++;
        }

        /*
         * Only flag a genuine write failure when BOTH reads disagree
         * with the expected value.  If either read matches expected,
         * the Flash cell was programmed correctly; the other read's
         * discrepancy is a read-path artefact (MISO pipeline).
         */
        if ((flash_verify_buf[i] != expected[i]) &&
            (flash_verify_buf2[i] != expected[i])) {
            if (g_page_verify_fail_count == 0UL) {
                g_page_verify_first_offset = (addr - SPI_FLASH_WAV_ADDR) + i;
                g_page_verify_first_expected = expected[i];
                g_page_verify_first_actual = flash_verify_buf[i];
                g_page_verify_first_actual2 = flash_verify_buf2[i];
                g_page_verify_first_missing_zero = flash_verify_buf2[i] & (~expected[i] & 0xFFU);
                g_page_verify_first_unwanted_zero = expected[i] & (~flash_verify_buf2[i] & 0xFFU);
            }
            g_page_verify_fail_count++;
            g_record_error_flags |= REC_ERR_PAGE_VERIFY;
            return;
        }
    }
}
...
static void flash_stream_flush(void)
{
    ...
    if (flash_page_len != 0UL) {
        page_index = (flash_page_addr - SPI_FLASH_WAV_ADDR) / SPI_FLASH_PAGE_BYTES;
        if (page_index < RECORD_FLASH_MAX_PAGES) {
            crc = crc32_update_buf(0xFFFFFFFFUL, flash_page_buf, flash_page_len);
            record_source_page_crc[page_index] = crc ^ 0xFFFFFFFFUL;
            record_source_page_len[page_index] = flash_page_len;
            if ((page_index + 1UL) > g_record_source_page_count) {
                g_record_source_page_count = page_index + 1UL;
            }
        }
        spi_flash_page_program(flash_page_addr, flash_page_buf, flash_page_len);
        /*
         * Immediately verify the written page against the source buffer.
         * If the SPI flash silently ignored a page-program chunk (e.g.
         * because WEL was lost or the chip was still busy from a previous
         * operation), this will catch it before the error propagates into
         * later CRC comparisons.
         */
        flash_stream_verify_page(flash_page_addr, flash_page_buf, flash_page_len);
...
{
    g_record_wav_crc = crc32_update_byte(g_record_wav_crc, value);
...
static uint8_t record_one_adpcm_block_to_flash(uint32_t block_index)
{
    uint32_t i;
    uint32_t word;
    uint32_t usedw;
    ...
    flash_stream_write_byte(0U);
    g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, 0U);
    g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, 0U);
    g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, 0U);
    g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, 0U);
    ...
    for (i = 0UL; i < (ADPCM_BLOCK_DATA_BYTES / 4UL); ++i) {
        if (!audio_read_adpcm_word_timeout(&word)) {
            g_record_error_flags |= REC_ERR_AUDIO_TIMEOUT;
            g_record_error_block = block_index;
            g_record_error_word = i;
            audio_stop();
            return 0U;
        }
        b0 = (uint8_t)(word & 0xFFUL);
        b1 = (uint8_t)((word >> 8) & 0xFFUL);
        b2 = (uint8_t)((word >> 16) & 0xFFUL);
        b3 = (uint8_t)((word >> 24) & 0xFFUL);
        flash_stream_write_byte(b0);
        flash_stream_write_byte(b1);
        flash_stream_write_byte(b2);
        flash_stream_write_byte(b3);
        g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, b0);
        g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, b1);
        g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, b2);
        g_record_adpcm_crc = crc32_update_byte(g_record_adpcm_crc, b3);
        /* Snapshot FIFO level AFTER popping — usedw reflects remaining words.
         * The true peak is (usedw + 1) for the word we just popped, but the
         * raw usedw post-pop is still a valid diagnostic: if it ever approaches
         * 1023 (FIFO depth - 1), overflow was imminent. */
        usedw = audio_fifo_usedw();
        if (usedw > g_fifo_max_usedw) {
            g_fifo_max_usedw = usedw;
        }
    }
...
static uint32_t debug_read_one_adpcm_block_crc(void)
{
    uint32_t i;
    uint32_t word;
    uint32_t usedw;
...
static void host_send_adpcm_debug_result(uint32_t status,
                                         uint32_t word_count,
                                         uint32_t crc,
                                         uint32_t fifo_max,
                                         uint32_t error_word)
{
    ...
    host_send_u32_le(word_count);
    host_send_u32_le(crc);
    host_send_u32_le(fifo_max);
    host_send_u32_le(error_word);
}

static uint32_t debug_capture_one_adpcm_block_to_ram(void)
{
    uint32_t i;
    uint32_t pos;
    uint32_t word;
    ...
    pos = 0UL;
    adpcm_debug_block[pos++] = 0U;
    adpcm_debug_block[pos++] = 0U;
    adpcm_debug_block[pos++] = 0U;
    adpcm_debug_block[pos++] = 0U;

    crc = 0xFFFFFFFFUL;
    for (i = 0UL; i < ADPCM_BLOCK_HEADER_BYTES; ++i) {
        crc = crc32_update_byte(crc, 0U);
    }
    ...
    for (i = 0UL; i < (ADPCM_BLOCK_DATA_BYTES / 4UL); ++i) {
        ...
        b2 = (uint8_t)((word >> 16) & 0xFFUL);
        b3 = (uint8_t)((word >> 24) & 0xFFUL);
        adpcm_debug_block[pos++] = b0;
        adpcm_debug_block[pos++] = b1;
        adpcm_debug_block[pos++] = b2;
        adpcm_debug_block[pos++] = b3;
        crc = crc32_update_byte(crc, b0);
        crc = crc32_update_byte(crc, b1);
        crc = crc32_update_byte(crc, b2);
        crc = crc32_update_byte(crc, b3);
...
static uint32_t debug_write_ram_adpcm_block_to_flash(void)
{
    uint32_t erase_bytes;
    uint32_t flash_crc;
    uint32_t offset;
    ...
    erase_bytes = SPI_FLASH_ERASE_BYTES;
    g_debug_erase_bad_count = 0UL;
    g_debug_first_mismatch_offset = 0xFFFFFFFFUL;
    g_debug_first_expected = 0UL;
    g_debug_first_actual = 0UL;
    g_debug_first_actual2 = 0UL;
    g_debug_first_wren_sr = 0UL;
    g_debug_first_pp_sr = 0UL;
    g_debug_first_wait_sr = 0UL;
    w25_read_jedec_raw_debug();

    spi_flash_erase_range(SPI_FLASH_WAV_ADDR, erase_bytes);

    offset = 0UL;
    while (offset < ADPCM_BLOCK_ALIGN) {
        chunk = min_u32(SPI_FLASH_PAGE_BYTES, ADPCM_BLOCK_ALIGN - offset);
        spi_flash_read_stable(SPI_FLASH_WAV_ADDR + offset, flash_page_buf, chunk);
        for (i = 0UL; i < chunk; ++i) {
            if (flash_page_buf[i] != 0xFFU) {
                g_debug_erase_bad_count++;
                if (g_debug_first_mismatch_offset == 0xFFFFFFFFUL) {
                    g_debug_first_mismatch_offset = offset + i;
                    g_debug_first_expected = 0xFFU;
                    g_debug_first_actual = flash_page_buf[i];
                    g_debug_first_actual2 = flash_page_buf[i];
                }
            }
        }
        offset += chunk;
    ...
    spi_flash_page_program(SPI_FLASH_WAV_ADDR, adpcm_debug_block, ADPCM_BLOCK_ALIGN);

    flash_crc = 0xFFFFFFFFUL;
    offset = 0UL;
    while (offset < ADPCM_BLOCK_ALIGN) {
        chunk = min_u32(SPI_FLASH_PAGE_BYTES, ADPCM_BLOCK_ALIGN - offset);
        spi_flash_read_stable(SPI_FLASH_WAV_ADDR + offset, flash_page_buf, chunk);
        spi_flash_read_stable(SPI_FLASH_WAV_ADDR + offset, flash_verify_buf2, chunk);
        for (i = 0UL; i < chunk; ++i) {
            flash_crc = crc32_update_byte(flash_crc, flash_page_buf[i]);
            if (flash_page_buf[i] != adpcm_debug_block[offset + i]) {
                if ((g_debug_first_mismatch_offset == 0xFFFFFFFFUL) ||
                    (g_debug_first_expected == 0xFFUL)) {
                    g_debug_first_mismatch_offset = offset + i;
                    g_debug_first_expected = adpcm_debug_block[offset + i];
                    g_debug_first_actual = flash_page_buf[i];
                    g_debug_first_actual2 = flash_verify_buf2[i];
                }
            }
        }
        offset += chunk;
    ...
    return flash_crc ^ 0xFFFFFFFFUL;
}

static void host_send_adpcm_flash_block_result(uint32_t status,
                                               ...
                                               uint32_t fifo_max)
{
    host_send_tag4("BCRC");
    host_send_u32_le(status);
    host_send_u32_le(block_bytes);
    host_send_u32_le(source_crc);
    host_send_u32_le(flash_crc);
    host_send_u32_le(fifo_max);
    host_send_u32_le(g_debug_erase_bad_count);
    host_send_u32_le(g_debug_first_mismatch_offset);
    host_send_u32_le(g_debug_first_expected);
    host_send_u32_le(g_debug_first_actual);
    host_send_u32_le(g_debug_first_actual2);
    host_send_u32_le(g_debug_first_wren_sr);
    host_send_u32_le(g_debug_first_pp_sr);
    host_send_u32_le(g_debug_first_wait_sr);
    host_send_u32_le(g_debug_jedec0);
    host_send_u32_le(g_debug_jedec1);
    host_send_u32_le(g_debug_jedec2);
    host_send_u32_le(g_debug_jedec3);
    host_send_u32_le(g_debug_jedec4);
    host_send_u32_le(g_debug_jedec5);
    host_send_u32_le(g_debug_jedec6);
    host_send_u32_le(g_debug_jedec7);
    host_send_u32_le(g_debug_sr_before_clear);
    host_send_u32_le(g_debug_sr_after_clear);
}

static void flash_stream_write_adpcm_wav_header(uint32_t block_count)
{
    uint32_t data_bytes;
    ...
    flash_stream_write((const uint8_t *)"RIFF", 4UL);
    flash_stream_write_le32(52UL + data_bytes);
    flash_stream_write((const uint8_t *)"WAVE", 4UL);

    flash_stream_write((const uint8_t *)"fmt ", 4UL);
    flash_stream_write_le32(20UL);
    flash_stream_write_le16(IMA_ADPCM_FORMAT_TAG);
    flash_stream_write_le16(WAV_CHANNELS);
    flash_stream_write_le32(WAV_SAMPLE_RATE_HZ);
    flash_stream_write_le32(avg_bytes_per_sec);
    flash_stream_write_le16((uint16_t)ADPCM_BLOCK_ALIGN);
    flash_stream_write_le16(IMA_ADPCM_BITS_PER_SAMPLE);
    flash_stream_write_le16(2U);
    flash_stream_write_le16((uint16_t)ADPCM_SAMPLES_PER_BLOCK);

    flash_stream_write((const uint8_t *)"fact", 4UL);
    flash_stream_write_le32(4UL);
    flash_stream_write_le32(sample_count);

    flash_stream_write((const uint8_t *)"data", 4UL);
    flash_stream_write_le32(data_bytes);
}
...
static uint32_t record_compressed_wav_to_spi_flash(void)
{
    ...
    g_record_adpcm_crc = 0xFFFFFFFFUL;
    g_record_error_flags = 0UL;
    g_record_error_block = 0UL;
    g_record_error_word = 0UL;
    ...
    spi_flash_erase_range(SPI_FLASH_WAV_ADDR, erase_bytes);
    flash_stream_begin(SPI_FLASH_WAV_ADDR);
    flash_stream_write_adpcm_wav_header(blocks);

    host_send_tag4("REC0");

    for (block = 0UL; block < blocks; ++block) {
        if (!record_one_adpcm_block_to_flash(block)) {
            break;
        }
        host_send_tag4("BEND");
        host_send_u32_le(block);
    }
...
static void output_wav_from_spi_flash(uint32_t flash_addr, uint32_t wav_bytes)
{
    ...
    while (offset < wav_bytes) {
        ...
        spi_flash_read_stable(flash_addr + offset, buf, chunk);
...
{
    spi_flash_read_stable(flash_addr, header, 12UL);
...
static void host_send_audio_flash_crc_result(uint32_t status,
                                             uint32_t wav_bytes,
                                             uint32_t source_wav_crc,
                                             uint32_t flash_wav_crc,
                                             uint32_t source_adpcm_crc,
                                             uint32_t flash_adpcm_crc,
                                             uint32_t fifo_max)
{
    host_send_tag4("ACRC");
    host_send_u32_le(status);
    host_send_u32_le(wav_bytes);
    host_send_u32_le(source_wav_crc);
    host_send_u32_le(flash_wav_crc);
    host_send_u32_le(source_adpcm_crc);
    host_send_u32_le(flash_adpcm_crc);
    host_send_u32_le(fifo_max);
}

static void host_send_record_error(uint32_t error_flags,
                                   uint32_t block_index,
                                   uint32_t word_index)
{
    host_send_tag4("RERR");
    host_send_u32_le(error_flags);
    host_send_u32_le(block_index);
    host_send_u32_le(word_index);
}

static void host_send_page_verify_result(uint32_t fail_count,
                                         uint32_t first_offset,
                                         uint32_t first_expected,
                                         uint32_t first_actual,
                                         uint32_t first_missing_zero,
                                         uint32_t first_unwanted_zero,
                                         uint32_t read_unstable_count,
                                         uint32_t first_actual2)
{
    host_send_tag4("PVFY");
    host_send_u32_le(fail_count);
    host_send_u32_le(first_offset);
    host_send_u32_le(first_expected);
    host_send_u32_le(first_actual);
    host_send_u32_le(first_missing_zero);
    host_send_u32_le(first_unwanted_zero);
    host_send_u32_le(read_unstable_count);
    host_send_u32_le(first_actual2);
}

static void host_send_page_crc_result(void)
{
    host_send_tag4("PCRC");
    host_send_u32_le(g_record_source_page_count);
    host_send_u32_le(g_record_page_crc_fail_count);
    host_send_u32_le(g_record_first_bad_page);
    host_send_u32_le(g_record_first_bad_offset);
    host_send_u32_le(g_record_first_bad_len);
    host_send_u32_le(g_record_first_bad_source_crc);
    host_send_u32_le(g_record_first_bad_flash_crc);
}
...
static uint32_t crc32_spi_flash(uint32_t flash_addr, uint32_t len)
{
    ...
    while (offset < len) {
        ...
        spi_flash_read_stable(flash_addr + offset, flash_page_buf, chunk);
...
static void compare_record_flash_pages(void)
{
    uint32_t page;
    uint32_t len;
    uint32_t addr;
    ...
    for (page = 0UL; page < g_record_source_page_count; ++page) {
        len = record_source_page_len[page];
        addr = SPI_FLASH_WAV_ADDR + (page * SPI_FLASH_PAGE_BYTES);

        /*
         * Read each page twice into independent buffers and compare them
         * byte-by-byte before computing the CRC.  If the two reads
         * disagree the Flash read path is unstable; retry up to 3 times
         * and use only the stable result.  This catches the same SPI
         * MISO pipeline issues guarded against during write verification.
         */
        read_stable = 0U;
        for (retry = 0UL; retry < 3UL; ++retry) {
            spi_flash_read_stable(addr, flash_page_buf, len);
            spi_flash_read_stable(addr, flash_verify_buf2, len);
            read_stable = 1U;
            for (i = 0UL; i < len; ++i) {
                if (flash_page_buf[i] != flash_verify_buf2[i]) {
                    read_stable = 0U;
                    break;
                }
            }
        ...
        /* Use the last read (flash_verify_buf2) for the CRC if stable. */
        flash_crc = crc32_update_buf(0xFFFFFFFFUL, flash_page_buf, len) ^ 0xFFFFFFFFUL;
        if (!read_stable) {
            flash_crc2 = crc32_update_buf(0xFFFFFFFFUL, flash_verify_buf2, len) ^ 0xFFFFFFFFUL;
            /*
             * If even the CRCs of two unstable reads match each other AND
             * match the source, treat the page as passing.
             */
            if ((flash_crc == flash_crc2) &&
                (flash_crc == record_source_page_crc[page])) {
                read_stable = 1U;
            }
        }

        if (!read_stable || (flash_crc != record_source_page_crc[page])) {
            g_record_page_crc_fail_count++;
            if (g_record_first_bad_page == 0xFFFFFFFFUL) {
                g_record_first_bad_page = page;
                g_record_first_bad_offset = page * SPI_FLASH_PAGE_BYTES;
                g_record_first_bad_len = len;
                g_record_first_bad_source_crc = record_source_page_crc[page];
                g_record_first_bad_flash_crc = flash_crc;
            }
        }
    }
}
...
int main(void)
{
    ...
    uint32_t source_adpcm_crc;
    uint32_t flash_adpcm_crc;
    uint32_t blocking_error;
    uint32_t adpcm_debug_crc;
    uint32_t i;
#endif
...
#elif RUN_ADPCM_ONLY_DEBUG
    g_record_error_flags = 0UL;
    g_record_error_word = 0UL;
    flash_adpcm_crc = 0UL;
    host_send_tag4("REC0");
    adpcm_debug_crc = debug_capture_one_adpcm_block_to_ram();
    if (g_record_error_flags != 0UL) {
        verify_status = WAV_TEST_FAIL_FLAG_VALUE;
    } else {
        flash_adpcm_crc = debug_write_ram_adpcm_block_to_flash();
        if ((g_record_error_flags == 0UL) &&
            (adpcm_debug_crc == flash_adpcm_crc)) {
            verify_status = DONE_FLAG_VALUE;
        } else {
            verify_status = WAV_TEST_FAIL_FLAG_VALUE;
        }
    }
    host_send_adpcm_debug_result(verify_status,
                                 ADPCM_BLOCK_DATA_BYTES / 4UL,
                                 adpcm_debug_crc,
                                 g_fifo_max_usedw,
                                 g_record_error_word);
    host_send_adpcm_flash_block_result(verify_status,
                                       ADPCM_BLOCK_ALIGN,
                                       adpcm_debug_crc,
                                       flash_adpcm_crc,
                                       g_fifo_max_usedw);
#else
    wav_bytes = record_compressed_wav_to_spi_flash();

    *done_fifo_max = g_fifo_max_usedw;
    *done_size = wav_bytes;
    source_wav_crc = g_record_wav_crc;
    source_adpcm_crc = g_record_adpcm_crc;
    blocking_error = g_record_error_flags & (REC_ERR_AUDIO_TIMEOUT | REC_ERR_FLASH_TIMEOUT);
    if (blocking_error != 0UL) {
        flash_wav_crc = 0UL;
        flash_adpcm_crc = 0UL;
        for (i = 0UL; i < 12UL; ++i) {
            wav_probe[i] = 0U;
        }
        *done_flag = WAV_TEST_FAIL_FLAG_VALUE;
        verify_status = WAV_TEST_FAIL_FLAG_VALUE;
    } else {
        flash_wav_crc = crc32_spi_flash(SPI_FLASH_WAV_ADDR, wav_bytes);
        flash_adpcm_crc = crc32_spi_flash(SPI_FLASH_WAV_ADDR + ADPCM_WAV_HEADER_BYTES,
                                          wav_bytes - ADPCM_WAV_HEADER_BYTES);
        compare_record_flash_pages();
        if (spi_flash_read_wav_probe(SPI_FLASH_WAV_ADDR, wav_probe)) {
        ...
        if ((source_wav_crc != flash_wav_crc) ||
            (source_adpcm_crc != flash_adpcm_crc) ||
            (g_record_page_crc_fail_count != 0UL)) {
            *done_flag = WAV_TEST_FAIL_FLAG_VALUE;
            verify_status = WAV_TEST_FAIL_FLAG_VALUE;
        }
    }
    *done_source_crc = source_wav_crc;
    *done_flash_crc = flash_wav_crc;
    host_send_flash_verify_probe(verify_status, wav_probe);

    host_send_tag4("FMAX");
    host_send_u32_le(g_fifo_max_usedw);

    if (g_record_error_flags != 0UL) {
        host_send_record_error(g_record_error_flags,
                               g_record_error_block,
                               g_record_error_word);
    }
    host_send_page_verify_result(g_page_verify_fail_count,
                                 g_page_verify_first_offset,
                                 g_page_verify_first_expected,
                                 g_page_verify_first_actual,
                                 g_page_verify_first_missing_zero,
                                 g_page_verify_first_unwanted_zero,
                                 g_page_verify_read_unstable_count,
                                 g_page_verify_first_actual2);
    host_send_page_crc_result();

    host_send_audio_flash_crc_result(verify_status,
                                     wav_bytes,
                                     source_wav_crc,
                                     flash_wav_crc,
                                     source_adpcm_crc,
    ...
    /*
     * Send the complete WAV file over UART for playback verification.
     * Sending 196668 bytes at 115200 baud takes ~34 seconds (1 byte-time
     * pacing in host_uart_putc doubles the raw bit-time).  The PC-side
     * uart_receiver.py saves this to a .wav file after printing the ACRC
     * diagnostics.
     */
    output_wav_from_spi_flash(SPI_FLASH_WAV_ADDR, wav_bytes);
#endif

    while (1) {
        /* Test mode reports "WVRS"; recording CRC mode reports "ACRC". */
    }
}
